...
<div id="toolbar">
	<div class="btn-group">
		<button class="btn btn-default" id="add">
			<i class="glyphicon glyphicon-plus">新增地区</i>
		</button>
	</div>
	<!--
...
<script>
	...
	function deleteObject(id) {
		...
		if (confirm(msg) == true) {
			$.ajax({
				url : "${domain}/location/deleteLocationbyid",
				type : "POST",
	...
	function initListener() {
		...
		$('body').on('click', '.item-edit', function() {
			...
			$.ajax({
				url : '${domain}/location/getLocationbyid',
				type : "POST",
				contentType : "application/json",
				data : JSON.stringify({
					locationid : dataid
		...
		$('body').on('click', '#submitBtn', function() {
			if (isAdd) {
				editCustom('${domain}/location/addLocation', JSON.stringify({
					locationno : $('#locationno').val(),
					locationname : $('#locationname').val()
				}));
			} else {
				editCustom('${domain}/location/updateLocation', JSON.stringify({
					locationid : dataid,
					locationno : $('#locationno').val(),
					locationname : $('#locationname').val()
				}));
